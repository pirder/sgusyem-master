...
namespace MyProject
{
    public partial class FrmSelect : Form
    {
        ...
        public FrmSelect(int a, string name, int limit)
        {
            ...
            for (int i = 1980; i < 3000; i++)
                comboGrantDateEndYear.Items.Add(i);
            string sqlsearchall = "select * from patent_institute";                                   
            //ColImageGrant.Text = "查看";//设置了TEXT的值，
            ColImageGrant.UseColumnTextForButtonValue = false;//设置了这个属性
            //ColImageCertificate.Text = "查看";//设置了TEXT的值，
            ColImageCertificate.UseColumnTextForButtonValue = false;//设置了这个属性           
            DataTable dt3 = new DataTable();
            dt3 = Program.GetDataTable(sqlsearchall.ToString());
            ...
            for (int i = 0; i < dt3.Rows.Count; i++)
            {
                ...
                if (!dr3["grant_inform_image"].ToString().Equals("无"))
                {
                    dgvr.Cells[12].Value = "查看";
                    /*
                ...
                if (!dr3["certificate_image"].ToString().Equals("无"))
                {
                    dgvr.Cells[13].Value = "查看";
                    /*
                    DataGridViewTextBoxCell dgvbc = new DataGridViewTextBoxCell();
                    dgvbc.Value = "无";
                    dgvr.Cells[13] = dgvbc;
     ...
     /*   private void FrmSelect_Load(object sender, EventArgs e)
      ...
      /*  private void pictureBox3_Click(object sender, EventArgs e)
        ...
        private void dataGridView1_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
            string sqlsearchall = "select * from patent_institute";
            DataTable dt = new DataTable();
            dt = Program.GetDataTable(sqlsearchall.ToString());
            if (e.RowIndex == -1)
                return;
            DataRow dr1 = dt.Rows[dataGridView1.CurrentRow.Index];
            gifullname = dr1["grant_inform_image"].ToString();
            ...
            if (!this.dataGridView1.Rows[dataGridView1.CurrentRow.Index].Cells[dataGridView1.CurrentCell.ColumnIndex].Value.ToString().Equals("无"))
                  try
                  {
                      if(e.ColumnIndex == dataGridView1.Columns[12].Index)
                            System.Diagnostics.Process.Start(gifullname);
                      if (e.ColumnIndex == dataGridView1.Columns[13].Index)
                            System.Diagnostics.Process.Start(cifullname);
                  }
        ...
        private void selectimportexcelbtn_Click_1(object sender, EventArgs e)
        {
